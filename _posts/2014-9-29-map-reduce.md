...
This blog post was previously published on [Rpubs](http://rpubs.com/aammd/cfrac):
...
We recently read my favourite chapter, [Functionals](http://adv-r.had.co.nz/Functionals.html), where readers are challenged to read about some algorithms in [Structure and Interpretation of Computer Programs](http://mitpress.mit.edu/sicp/full-text/book/book-Z-H-12.html#%_sec_1.3), and implement it in R.

I wanted to share some functions I wrote to calculate these exotic things called _k-term finite continued fractions_, based on that challenge:
...
and have an infinite number of $n$ and $d$ values.  However, if after $k$ values you just replace the remaining ones (the $...$ above) with 0, then you get a _k-term finite continued fraction_, which is maybe close enough:
...
![plot of chunk picomparion](./continued_frac_map_reduce_files/figure-html/picomparion.png) 
...
![plot of chunk piplot](./continued_frac_map_reduce_files/figure-html/piplot.png) 
