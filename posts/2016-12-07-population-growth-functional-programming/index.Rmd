...
and there is! An answerer mentioned `purr::accumulate()`. In this post I'm going to expand on the approach they suggest. `accumulate`, and its twin `reduce`, are examples of _functionals_ -- functions that take functions as their arguments, and manipulate their behaviour in some way. `purrr::accumulate` is a wrapper around to `Reduce` from the base package, with the argument `accumulate = TRUE`. 
...
In the original question, [jebyrnes asked](http://stackoverflow.com/questions/40095463/simulating-a-timeseries-in-dplyr-instead-of-using-a-for-loop#comment67489125_40096347) if parameters could be allowed to vary over time. Here is a way of allowing that to happen. We have to get a bit more abstract, and write a function which takes both population growth rate `lambda` and the population at time N~t~ as inputs
```{r}
...
How do we run these little functions? We need yet another function -- this time, a function that takes two functions and _runs the second on the first_:
```{r}
