...
parameters {
  ...
  vector[nlinks] predation_effects;
}
transformed parameters {
  ...
  for (i in 1:nlinks){
        B[resc[i],cons[i]] = predation_effects[i];
        B[cons[i],resc[i]] = predation_effects[i];
    }
// print(B);
  // Growth rate
...
model {
  log_A ~ normal(1.5, .2);
  diag_B_logit ~ normal(-1, .5);
  log_comp ~ normal(-1.5, .2);
  predation_effects ~ normal(0, .1);
  // print(B);
  // print(A);
  // print(additions);
  // print(true_abd);
